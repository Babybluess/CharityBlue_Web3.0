...
contract MultiCrowdfunding {
    ...
    function changeOwner(address newOwner) public onlyCreator {
    ...
    function contribute(
        uint256 campaignIndex
    ) public payable validCampaignIndex(campaignIndex) {
        ...
        } else if (isUserVoted[msg.sender][campaignIndex]) {
    ...
    function vote(
        uint256 campaignIndex,
        bool support
    ) public validCampaignIndex(campaignIndex) {
        Campaign storage campaign = campaigns[campaignIndex];
        require(!campaign.isCompleted, "Crowdfunding is already completed.");
        ...
        require(
            contribution > 0,
            "You must have contributed to the campaign to vote."
        );

        require(
            !isUserVoted[msg.sender][campaignIndex],
    ...
    function withdrawFunds(
        uint256 campaignIndex
    ) public onlyCreator validCampaignIndex(campaignIndex) {
        Campaign storage campaign = campaigns[campaignIndex];
    ...
    function getCampaignDetails(
        ...
        public
        view
        validCampaignIndex(campaignIndex)
        returns (
            string memory name,
            uint256 goal,
            uint256 deadline,
        ...
        )
    {
        ...
        for (uint256 i = 0; i < contributors.length; i++) {
            contributions[i] = userContributions[contributors[i]][
                campaignIndex
            ];
        }
        ...
        returns (address[] memory, uint256[] memory)
    {
        ...
        for (uint256 i = 0; i < contributorCount; i++) {
            ...
            sortedContributions[i] = userContributions[
            ...
            for (uint256 j = i + 1; j < contributorCount; j++) {
                if (sortedContributions[i] < sortedContributions[j]) {
                    (sortedContributions[i], sortedContributions[j]) = (
                        sortedContributions[j],
                        sortedContributions[i]
                    );
                    (sortedContributors[i], sortedContributors[j]) = (
                        sortedContributors[j],
                        sortedContributors[i]
                    );
                }
            }
        }
